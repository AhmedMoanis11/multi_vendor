...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<DataState<String>> forgrtPasswordVerification(UserEntity user) async {
    if (await networkInfo.isConnected) {
      try {
        UserModel userModel =
            UserModel(verify_code: user.verify_code, email: user.email);
  ...
  Future<DataState<String>> resetPassword(UserEntity user) async {
    if (await networkInfo.isConnected) {
      try {
        UserModel userModel =
            UserModel(password: user.password, email: user.email);
  ...
  Future<DataState<String>> signUpVerification(UserEntity user) async {
    if (await networkInfo.isConnected) {
      try {
        ...
        String response = await authDataSources.signUpVerification(userModel);
